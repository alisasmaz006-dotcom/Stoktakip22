...
import RepairPartsPage from './pages/RepairPartsPage';
...
const viewLabels: Record<string, string> = {
  ...
  reminders: 'Hatırlatıcılar', appointments: 'Randevu Takvimi', warranty: 'Garanti Takibi',
  aiForecast: 'AI Tahmin', backup: 'Veri Yedekleme', repairParts: 'Parça Stoğu', settings: 'Genel Ayarlar',
};
...
function AppContent() {
  ...
  const loadAllData = useCallback(async () => {
    ...
    try {
      ...
      const results = await Promise.allSettled([
        api.getCategories(bid).then(setCategories),
        api.getProducts(bid).then(setProducts),
        api.getSales(bid).then(setSales),
        api.getRepairs(bid).then(setRepairs),
        api.getPhoneSales(bid).then(setPhoneSales),
        api.getPhoneStocks(bid).then(setPhoneStocks),
        api.getExpenses(bid).then(setExpenses),
        api.getCustomerRequests(bid).then(setRequests),
        api.getSuppliers(bid).then(setSuppliers),
        api.getPurchases(bid).then(setPurchases),
        api.getCustomers(bid).then(setCustomers).catch(() => { }),
      ]);
      results.forEach((r, i) => {
  ...
  // activeBranchId değişince verileri yenile
  useEffect(() => {
    loadAllData();
  }, [activeBranchId, loadAllData]);
  ...
  const renderView = () => {
    ...
    switch (activeView) {
      ...
      case 'suppliers': return <SuppliersPage suppliers={suppliers} setSuppliers={setSuppliers} repairs={repairs} expenses={expenses} setExpenses={setExpenses} />;
      case 'reminders': return <RemindersPage />;
      case 'appointments': return <AppointmentCalendar />;
      case 'warranty': return <WarrantyTracker />;
      case 'aiForecast': return <AIForecastWidget sales={sales} repairs={repairs} phoneSales={phoneSales} expenses={expenses} />;
      case 'backup': return <BackupManager />;
      case 'repairParts': return <RepairPartsPage />;
  ...
  return (
    <>
      <AppShell activeView={activeView} onViewChange={setActiveView} renderView={renderView} />
      <GlobalSearch products={products} customers={customers} repairs={repairs} suppliers={suppliers} onNavigate={(view) => setActiveView(view)} />
      <PWAInstallPrompt />
    </>
  );
}

function AppShell({ activeView, onViewChange, renderView }: { activeView: string; onViewChange: (v: string) => void; renderView: () => React.ReactNode }) {
  const { visible, toggle, currency, setCurrency, usdRate, rateLoading } = usePriceVisibility();
  const { activeBranch, viewMode } = useBranch();
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden overflow-x-hidden">
          ...
          <div className="flex items-center gap-2 md:gap-3 text-sm">
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              ...
              <span className="text-slate-400 text-xs ml-2 hidden md:inline">
                {viewMode === 'all' ? '— Tüm Şubeler' : activeBranch ? `— ${activeBranch.name}` : ''}
              </span>
            </div>
          </div>
        ...
        {/* Content */}
        <div key={activeView} className="flex-1 flex flex-col overflow-hidden animate-page-enter">
          {renderView()}
        </div>
      </main>
    </div>
