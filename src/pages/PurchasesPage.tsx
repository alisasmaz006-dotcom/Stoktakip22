    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (result?.id) {
                ...
                // Manually record cari hareketler (instead of DB trigger to avoid UUID type issues)
                try {
                    // A. Alış tahakkuku (borç artışı)
                    await api.saveCariHareket({
                        supplierId,
                        islemTarihi: purchaseDate,
                        islemTipi: 'alis',
                        miktar: grandTotal,
                        aciklama: notes || 'Alış Faturası',
                        ilgiliId: result.id as string,
                        faturaNo: invoiceNumber,
                        bakiyeEtkisi: grandTotal,
                        branchId: activeBranchId || undefined
                    });
                } catch (cariErr) {
                    // Non-critical: log but don't block the purchase save
                    console.warn('Cari hareket kaydedilemedi:', cariErr);
                }

                // Update product stocks via Edge Function (products are in KV store, not Supabase)
