        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-thin">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                    ...
                    { label: 'Toplam Stok Değeri', value: canSeePurchasePrice ? fp(totalValue) : '***', icon: 'payments', sub: 'Alış fiyatına göre', color: 'text-green-400', bgIcon: 'text-green-500' },
            ...
            <div className="bg-surface-dark border border-slate-700/50 rounded-xl overflow-x-auto">
                <table className="w-full text-left min-w-[800px]">
                    <thead><tr className="bg-slate-800/50 border-b border-slate-700 text-xs uppercase text-slate-400 font-semibold tracking-wider">
                        ...
                        {canSeePurchasePrice && <th className="p-4 text-right">Alış</th>}<th className="p-4 text-right">Satış</th><th className="p-4 w-32">Stok</th>
                        ...
                        {filtered.length === 0 ? (
                            ...
                            return (
                                <tr key={p.id} className="hover:bg-slate-800 transition-colors group">
                                    ...
                                    {canSeePurchasePrice && <td className="p-4 text-right text-slate-300">{fp(p.purchasePrice)}</td>}
            ...
            {/* Barcode Print Modal */}
            {showBarcodeModal && (
                <BarcodePrintModal products={products} onClose={() => setShowBarcodeModal(false)} />
            )}
