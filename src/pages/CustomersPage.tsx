...
type PeriodFilter = 'today' | 'thisMonth' | 'lastMonth' | 'all' | 'custom';
...
export default function CustomersPage({ repairs, phoneSales, sales, customers, setCustomers }: CustomersPageProps) {
    ...
    const [form, setForm] = useState({ name: '', phone: '', email: '', address: '', notes: '', photoUrl: '', idNumber: '' });
    ...
    const getEndDate = () => {
        if (period === 'today') return new Date(now.getFullYear(), now.getMonth(), now.getDate(), 23, 59, 59);
    ...
    const handleAddTransaction = async () => {
        ...
        try {
            ...
            const updated = { ...txCustomer, debt: newDebt, credit: newCredit, branchId: activeBranchId || undefined };
    ...
    const openCreate = () => {
        setEditing(null);
        setForm({ name: '', phone: '', email: '', address: '', notes: '', photoUrl: '', idNumber: '' });
        setShowModal(true);
    };

    const openEdit = (c: Customer) => {
        setEditing(c);
        setForm({ name: c.name, phone: c.phone || '', email: c.email || '', address: c.address || '', notes: c.notes || '', photoUrl: c.profilePhotoUrl || (c as any).photoUrl || '', idNumber: c.idNumber || (c as any).idNumber || '' });
        setShowModal(true);
    };
    ...
    const handlePhotoUpload = async (file: File) => {
        ...
        try {
            const url = await uploadPhoto('customer-photos', file);
    ...
    const handleSave = async () => {
        ...
        try {
            const payload = { ...form, profilePhotoUrl: form.photoUrl, idNumber: form.idNumber, branchId: activeBranchId || undefined };
            if (editing) {
                const result = await api.saveCustomer({ ...editing, ...payload });
                if (result) setCustomers(customers.map(c => c.id === editing.id ? { ...editing, ...payload, ...(result as unknown as Customer) } : c));
            } else {
                const result = await api.saveCustomer(payload);
    ...
    const importFromTransactions = async () => {
        ...
        try {
            ...
            for (const entry of entries) {
                try {
                    const result = await api.saveCustomer({ name: entry.name, phone: entry.phone, email: '', address: '', notes: '', branchId: activeBranchId || undefined });
                ...
                <div className={`flex-1 overflow-y-auto p-6 space-y-6 scrollbar-thin transition-all ${selectedCustomer ? 'pr-0' : ''}`}>
                    ...
                    <div className="flex justify-between items-center bg-surface-dark border border-slate-700/50 p-4 rounded-xl">
                        ...
                        <div className="flex items-center gap-2">
                            {([['today', 'Bugün'], ['thisMonth', 'Bu Ay'], ['lastMonth', 'Geçen Ay'], ['all', 'Tüm Zamanlar']] as const).map(([id, label]) => (
                                <button key={id}
                    ...
                    <div className="w-96 border-l border-slate-700 bg-surface-dark/80 overflow-y-auto scrollbar-thin animate-fade-in flex-shrink-0">
                        ...
                        <div className="p-4 space-y-5">
                            ...
                            <div className="bg-gradient-to-br from-violet-500/10 to-purple-500/10 border border-violet-500/20 rounded-xl p-5 text-center">
                                <div className="w-16 h-16 rounded-full bg-violet-500/20 flex items-center justify-center mx-auto mb-3 overflow-hidden">
                                    {selectedCustomer.profilePhotoUrl || (selectedCustomer as any).photoUrl ? (
                                        <img src={selectedCustomer.profilePhotoUrl || (selectedCustomer as any).photoUrl} alt={selectedCustomer.name} className="w-full h-full object-cover" />
                                    ) : (
                                ...
                                {selectedCustomer.idNumber && (
                                    <p className="text-xs text-slate-500 mt-1 flex items-center justify-center gap-1">
                                        ...
                                        TC/Kimlik: {selectedCustomer.idNumber}
                                    </p>
                                )}
                            </div>
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setShowModal(false)}>
                    <div className="bg-surface-dark border border-slate-700 rounded-2xl w-[95vw] md:w-full md:max-w-lg animate-fade-in" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div><label className="block text-sm font-medium text-slate-300 mb-1">
                                    <span className="flex items-center gap-1"><span className="material-symbols-outlined text-sm">badge</span>TC/Kimlik No</span>
                                </label>
                                    <input type="text" value={form.idNumber} onChange={e => setForm({ ...form, idNumber: e.target.value })}
                                        className="w-full bg-slate-800 border border-slate-700 rounded-lg py-2 px-3 text-sm text-white focus:border-violet-500 outline-none" placeholder="TC Kimlik / Pasaport No" /></div>
                                <div><label className="block text-sm font-medium text-slate-300 mb-1">
                                    <span className="flex items-center gap-1"><span className="material-symbols-outlined text-sm">photo_camera</span>Müşteri Fotoğrafı</span>
                                </label>
                                    <div className="flex items-center gap-2">
                                        <input type="file" accept="image/*" onChange={e => {
                                            if (e.target.files && e.target.files.length > 0) handlePhotoUpload(e.target.files[0]);
                                        }} className="hidden" id="customer-photo-upload" />
                                        <label htmlFor="customer-photo-upload" className="flex-1 cursor-pointer bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-lg py-2 px-3 text-sm text-center text-slate-300 transition-colors flex justify-center items-center gap-2">
                                            {uploadingPhoto ? (
                                                <><span className="animate-spin h-4 w-4 border-2 border-violet-500 border-t-transparent rounded-full"></span>Yükleniyor...</>
                                            ) : (
                                                <><span className="material-symbols-outlined text-sm">upload</span>Resim Seç</>
                                            )}
                                        </label>
                                        {form.photoUrl && (
                                            <button type="button" onClick={() => setForm({ ...form, photoUrl: '' })} className="p-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-lg">
                            ...
                            {form.photoUrl && (
                                <div className="flex justify-center p-3 bg-slate-800/50 rounded-xl">
                                    <img src={form.photoUrl} alt="Önizleme" className="w-24 h-24 rounded-full object-cover border-4 border-violet-500/30" onError={e => (e.currentTarget.style.display = 'none')} />
                                </div>
                            )}
