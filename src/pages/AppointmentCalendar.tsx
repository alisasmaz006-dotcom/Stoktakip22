...
export default function AppointmentCalendar() {
    ...
    const handleSave = async () => {
        ...
        await api.saveAppointment({ ...form, ...(editingApt?.id ? { id: editingApt.id } : {}), technicianName: tech?.name || '', branchId: activeBranchId || undefined });
        setShowForm(false);
        setEditingApt(null);
    ...
    const handleStatusChange = async (apt: RepairAppointment, status: string) => {
        await api.saveAppointment({ ...apt, status, branchId: activeBranchId || undefined });
        loadData();
    };
