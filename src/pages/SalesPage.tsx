...
export default function SalesPage({ sales, repairs, phoneSales, products, setSales, onRefresh, customers, setCustomers }: SalesPageProps) {
    const { showToast } = useToast();
    const fp = useFormatPrice();
    const { branchSettings, activeBranchId } = useBranch();
    const { isOwner } = useAuth();
    const canSeePurchasePrice = isOwner || (branchSettings?.showPurchasePrice ?? true);
    ...
    const filteredRepairs = useMemo(() =>
        ...
        }), [repairs, period, customStart, customEnd]);
    ...
    // Customer Cari Balance (Alacaklarımız - Borçlarımız) => Pozitif ise lehimize 
    const totalCustomerDebt = customers.reduce((s, c) => s + (c.debt || 0), 0);
    const totalCustomerCredit = customers.reduce((s, c) => s + (c.credit || 0), 0);
    const cariBalance = totalCustomerDebt - totalCustomerCredit;
    ...
    const handleSaveSale = async () => {
        ...
        try {
            ...
            if (salePaymentMethod === 'card' && commissionRate > 0) {
                const commissionAmount = totalPrice * (commissionRate / 100);
                finalProfit -= commissionAmount;
            }

            const sale: Sale = {
                id: '', items, totalPrice, totalProfit: finalProfit,
    ...
    const handleDeleteSale = async (id: string) => {
        if (!confirm('Bu satışı silmek ve stokları geri almak istediğinize emin misiniz?')) return;
        try {
            ...
            if (saleToDelete) {
                ...
                for (const item of saleToDelete.items) {
                    const prod = products.find(p => p.id === item.productId);
                    if (prod) {
                        await api.saveProduct({ ...prod, stock: prod.stock + item.quantity });
                    }
                }
            }
            await api.deleteSale(id);
            setSales(sales.filter(s => s.id !== id));
            await onRefresh(); // Re-fetch products to reflect updated stock
            showToast('Satış silindi ve stoklar iade edildi!');
        } catch {
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-thin">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                    ...
                    { label: 'Cari Bakiye', value: fp(Math.abs(cariBalance)), icon: 'account_balance', color: cariBalance >= 0 ? 'text-emerald-400' : 'text-red-400', bgIcon: 'text-orange-500' },
                ].map(card => (
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-left min-w-[800px]">
                                <thead><tr className="bg-slate-800/50 border-b border-slate-700 text-xs uppercase text-slate-400 font-semibold tracking-wider">
                                    ...
                                    <th className="p-3">Ödeme</th><th className="p-3 text-right">Tutar</th>{canSeePurchasePrice && <th className="p-3 text-right">Kâr</th>}<th className="p-3 text-center">İşlemler</th>
                                </tr></thead>
                                <tbody className="divide-y divide-slate-700/50 text-sm">
                                    {searchedSales.length === 0 ? (
                                        ...
                                        <tr key={s.id} className="hover:bg-surface-hover/50 transition-colors">
                                            ...
                                            {canSeePurchasePrice && <td className="p-3 text-right font-medium text-emerald-400">+{fp(s.totalProfit)}</td>}
